...
struct fake_cfg {
  ...
  auto on(ut::events::test<Ts...> test) -> void {
    ...
    if (std::empty(test_filter) || std::string_view{test.name} == test_filter) {
...
int main() {
  {
...

    {
      ...
      test_assert(!utility::is_match("", "abc"));
      test_assert(!utility::is_match("abc", "b??"));
      test_assert(!utility::is_match("abc", "a*d"));
      test_assert(!utility::is_match("abc", "*C"));
    }
    ...
    {
      static_assert(!std::is_same_v<decltype(_sc(42)), decltype(_c(42))>);
      static_assert(!std::is_same_v<decltype(42_sc), decltype(42_c)>);
      static_assert(!std::is_same_v<decltype(_sc(42)), decltype(_uc(42u))>);
      static_assert(!std::is_same_v<decltype(42_sc), decltype(42_uc)>);
    }

    {
      static_assert(_i(42) == 42_i);
      static_assert(_b(true));
      static_assert(!_b(false));
...

    {
      ...
      test_assert("0 <= 0" == to_string(0_s <= _s(0)));
      test_assert("0 != 0" == to_string(0_s != _s(0)));
    // FIXME: Fails in windows
    //   test_assert("!true" == to_string(!"true"_b));
      ...
      test_assert("(true or 42.42 == 12.34)" ==
                  to_string("true"_b || (42.42_d == 12.34)));
    // FIXME: Fails in windows
    //   test_assert("(!1 == 2 and str == str2)" ==
    //               to_string(!(1_i == 2) && ("str"sv == "str2"sv)));
...

    {
      ...
      reporter.on(events::assertion_pass<bool>{
          .expr = true,
          .location =
              std::source_location::current(42, {}, "file/name.cpp")});
...

    {
      ...
      run.on(events::skip<>{.type = "test", .name = "skip", .arg = boost::tmp::nothing_{}});
      test_assert(1 == reporter.tests_.pass);
      test_assert(0 == reporter.tests_.fail);
      test_assert(1 == reporter.tests_.skip);
...

    {
      ...
      void(std::any_cast<boost::tmp::nothing_>(test_cfg.run_calls[0].arg));
...

    {
      ...
      void(std::any_cast<boost::tmp::nothing_>(test_cfg.skip_calls[0].arg));
      test_assert(std::empty(test_cfg.log_calls));
      test_assert(0 == test_cfg.fatal_assertion_calls);
    }
...

    {
      ...
      test_assert("false msg"sv ==
                  std::any_cast<const char*>(test_cfg.log_calls[1]));
      test_assert(' ' == std::any_cast<char>(test_cfg.log_calls[2]));
      test_assert(!std::any_cast<bool>(test_cfg.log_calls[3]));
    }
...

    {
...

      {
        constexpr auto c = 4;
        auto r = 2;
        expect(constant<4_i == c> && r == 2_i);
      }

      {
        constexpr auto c = 5;
        auto r = 2;
        expect(constant<4_i == c> && r == 2_i) << "fail compile-time";
      }

      {
        constexpr auto c = 4;
        auto r = 3;
        expect(constant<4_i == c> && r == 2_i) << "fail run-time";
      }
...

    {
      ...
      test_assert(!test_cfg.assertion_calls[8].result);
    }
...

    {
      ...
      expect(_t{custom{1}} == custom{2} || 1_i == 22);
...

    {
      ...
      "args string"_test = [](const auto& arg) {
        expect(_c('s') == arg || _c('t') == arg || _c('r') == arg);
      } | std::string{"str"};
...

    {
      ...
      "args and types"_test = []<class TArg>(const TArg& arg) {
        expect(42_i == arg || arg == _c('x'));
        expect(type<TArg> == type<int> || type<TArg> == type<char>);
      } | std::tuple{42, 'x'};

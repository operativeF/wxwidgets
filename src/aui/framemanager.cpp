...
using wxAuiRectArray = std::vector<wxRect>;
                                ...
                                wxAuiDockInfo* ex_cept  = nullptr  )
{
    for (size_t i = 0; i < docks.size(); ++i)
    {
        ...
        if (pi)
            std::erase(d.panes, pi); // FIXME: Delete memory too?
    }
}
...
{
    int i, dock_count;
    for (i = 0, dock_count = docks.size(); i < dock_count; ++i)
    {
        wxAuiDockInfo& dock = docks[i];
        dock.dock_row = i;

        int j, pane_count;
        for (j = 0, pane_count = dock.panes.size(); j < pane_count; ++j)
...
//     wxAuiToolBar* toolbar = dynamic_cast<wxAuiToolBar*>(window);
...
wxAuiDockUIPart* wxAuiManager::HitTest(int x, int y)
{
    ...
    {
        wxAuiDockUIPart& item = m_uiParts[i];
...
{
    wxASSERT_MSG(wnd, "specified window must be non-NULL");
    ...
    if (dynamic_cast<wxMDIParentFrame*>(m_frame))
    {
        ...
        AddPane(client_window,
                wxAuiPaneInfo().Name("mdiclient").
                CenterPane().PaneBorder(false));
    }
    else if (dynamic_cast<wxAuiMDIParentFrame*>(m_frame))
    {
...
{
    wxASSERT_MSG(window, "NULL window ptrs are not allowed");
    ...
    m_panes.push_back(test);

    wxAuiPaneInfo& pinfo = m_panes.back();
    ...
    {
        pinfo.name = fmt::format("%08lx%08x%08x%08lx",
             static_cast<unsigned long>(wxPtrToUInt(pinfo.window) & 0xffffffff),
             static_cast<unsigned int>(time(nullptr)),
             static_cast<unsigned int>(clock()),
             static_cast<unsigned long>(m_panes.size()));
    }
    ...
    if (pinfo.HasGripper())
    {
        if (dynamic_cast<wxAuiToolBar*>(pinfo.window))
        {
...
#if wxUSE_TOOLBAR
        if (dynamic_cast<wxToolBar*>(pinfo.window))
        {
...
bool wxAuiManager::DetachPane(wxWindow* window)
{
    ...
    for (size_t i = 0; i < m_panes.size(); ++i)
    {
        ...
        if (p.window == window)
        {
            ...
            int pi, part_count;
            for (pi = 0, part_count = (int)m_uiParts.size(); pi < part_count; ++pi)
            {
                wxAuiDockUIPart& part = m_uiParts[pi];
                if (part.pane == &p)
                {
                    m_uiParts.erase(m_uiParts.begin() + pi);
                    part_count--;
                    pi--;
                    continue;
                }
...
static std::string EscapeDelimiters(const std::string& s)
{
    std::string result;
...
std::string wxAuiManager::SavePaneInfo(const wxAuiPaneInfo& pane)
{
    std::string result = fmt::format("name={};caption={};", EscapeDelimiters(pane.name), EscapeDelimiters(pane.caption));

    result += fmt::format("state=%u;", pane.state);
    result += fmt::format("dir=%d;", pane.dock_direction);
    result += fmt::format("layer=%d;", pane.dock_layer);
    result += fmt::format("row=%d;", pane.dock_row);
    result += fmt::format("pos=%d;", pane.dock_pos);
    result += fmt::format("prop=%d;", pane.dock_proportion);
    result += fmt::format("bestw=%d;", pane.best_size.x);
    result += fmt::format("besth=%d;", pane.best_size.y);
    result += fmt::format("minw=%d;", pane.min_size.x);
    result += fmt::format("minh=%d;", pane.min_size.y);
    result += fmt::format("maxw=%d;", pane.max_size.x);
    result += fmt::format("maxh=%d;", pane.max_size.y);
    result += fmt::format("floatx=%d;", pane.floating_pos.x);
    result += fmt::format("floaty=%d;", pane.floating_pos.y);
    result += fmt::format("floatw=%d;", pane.floating_size.x);
    result += fmt::format("floath=%d", pane.floating_size.y);

    return result;
}

// Load a "pane" with the pane information settings in pane_part
void wxAuiManager::LoadPaneInfo(std::string pane_part, wxAuiPaneInfo &pane)
{
    // replace escaped characters so we can
    // split up the string easily
    wx::utils::ReplaceAll(pane_part, "\\|", "\a");
    wx::utils::ReplaceAll(pane_part, "\\;", "\b");

    while(true)
    {
        auto val_part = wx::utils::BeforeFirst(pane_part, ";");
        pane_part = wx::utils::AfterFirst(pane_part, ";");

        auto val_name = wx::utils::BeforeFirst(val_part, "=");
        auto value    = wx::utils::AfterFirst(val_part, "=");

        wx::utils::ToLower(val_name);
        wx::utils::TrimAllSpace(val_name);
        wx::utils::TrimAllSpace(value);
    ...
    // replace escaped characters so we can
    // split up the string easily
    wx::utils::ReplaceAll(pane.name, "\a", "|");
    wx::utils::ReplaceAll(pane.name, "\b", ";");

    wx::utils::ReplaceAll(pane.caption, "\a", "|");
    wx::utils::ReplaceAll(pane.caption, "\b", ";");
...
std::string wxAuiManager::SavePerspective()
{
    std::string result;
    result.reserve(500);
    result = "layout2|";

    for (std::size_t pane_i{0}; pane_i != m_panes.size(); ++pane_i)
    {
        wxAuiPaneInfo& pane = m_panes[pane_i];
        result += SavePaneInfo(pane) + "|";
    }

    for (std::size_t dock_i{0}; dock_i != m_docks.size(); ++dock_i)
    {
        wxAuiDockInfo& dock = m_docks[dock_i];

        result += fmt::format("dock_size(%d,%d,%d)=%d|",
                               dock.dock_direction,
                               dock.dock_layer,
                               dock.dock_row,
                               dock.size);
...
bool wxAuiManager::LoadPerspective(const std::string& layout, bool update)
{
    ...
    int pane_i, pane_count = m_panes.size();
    for (pane_i = 0; pane_i < pane_count; ++pane_i)
    {
        wxAuiPaneInfo& p = m_panes[pane_i];
        if(p.IsDockable())
            p.Dock();
        p.Hide();
    }
    ...
    // replace escaped characters so we can
    // split up the string easily
    wx::utils::ReplaceAll(input, "\\|", "\a");
    wx::utils::ReplaceAll(input, "\\;", "\b");
    ...
    while (true)
    {
        ...
        std::string pane_part = wx::utils::BeforeFirst(input, "|");
        input = wx::utils::AfterFirst(input, "|");

        wx::utils::TrimTrailingSpace(pane_part);

        // if the string is empty, we're done parsing
        if (pane_part.empty())
            break;

        if (pane_part.substr(0, 9) == "dock_size")
        {
            auto val_name = wx::utils::BeforeFirst(pane_part, "=");
            auto value    = wx::utils::AfterFirst(pane_part, "=");

            long dir, layer, row, size;

            auto piece = wx::utils::AfterFirst(val_name, "(");
            piece = wx::utils::BeforeLast(piece, ")");

            auto dir_val = wx::utils::BeforeFirst(piece, ",");

            piece = wx::utils::AfterFirst(piece, ",");
                                            ...
                                            std::vector<int>& sizes)
{
    ...
    int action_pane = -1;
    int pane_i, pane_count = dock.panes.size();

    // find the pane marked as our action pane
    for (pane_i = 0; pane_i < pane_count; ++pane_i)
    {
        wxAuiPaneInfo& pane = *(dock.panes[pane_i]);
                                 ...
                                 bool spacer_only)
{
    ...
    if (pane.HasCaption())
    {
        ...
        part.sizer_item = sizer_item;
        int caption_part_idx = uiparts.size();
        uiparts.push_back(part);
    ...
    if (min_size != wxDefaultSize)
    {
        vert_pane_sizer->SetItemMinSize(
                        vert_pane_sizer->GetChildren().size() - 1,
                        min_size);
    }
                                 ...
                                 bool spacer_only)
{
    ...
    int pane_i;
    auto pane_count = dock.panes.size();
                                 ...
                                 bool spacer_only)
{
    ...
    for (size_t i = 0; i < docks.size(); ++i)
    {
        ...
        std::sort(dock.panes.begin(), dock.panes.end(), PaneSortFunc);
        ...
        if (dock.size == 0)
        {
            ...
            for (size_t j = 0; j != dock.panes.size(); ++j)
            {
                wxAuiPaneInfo& pane = *dock.panes[j];
    ...
    for (layer = 0; layer <= max_layer; ++layer)
    {
        ...
        if (!arr.empty())
        {
            for (row = 0, row_count = arr.size(); row < row_count; ++row)
                LayoutAddDock(cont, *arr[row], uiparts, spacer_only);
        }
        ...
        {
            for (row = 0, row_count = arr.size(); row < row_count; ++row)
                LayoutAddDock(middle, *arr[row], uiparts, spacer_only);
        }
        ...
        if (!arr.empty())
        {
            for (row = arr.size() - 1; row >= 0; --row)
                LayoutAddDock(middle, *arr[row], uiparts, spacer_only);
        }

        if (middle->GetChildren().size() > 0)
                          ...
                          const wxPoint& offset)
{
        ...
        part->type == wxAuiDockUIPart::typeBackground)
    {
        ...
        {
            if (part->dock->panes.size() != 1)
                return false;
            part = GetPanePart(part->dock->panes[0]->window);
            if (!part)
                return false;
        }
...
void wxAuiManager::ShowHint(const wxRect& rect)
{
    if (m_hintWnd)
    {
        ...
        if ((m_flags & wxAUI_MGR_HINT_FADE)
            && !((dynamic_cast<wxPseudoTransparentFrame*>(m_hintWnd)) &&
                                       ...
                                       const wxPoint& offset)
{
    ...
    if ( m_frame->GetLayoutDirection() == wxLayoutDirection::RightToLeft )
    {
...
void wxAuiManager::OnFindManager(wxAuiManagerEvent& evt)
{
    ...
    // FIXME: stupid.
    if (dynamic_cast<wxAuiFloatingFrame*>(window))
    {
    ...
    if (part)
    {
                  ...
                  part->type == wxAuiDockUIPart::typeGripper)
        {
            ...
            if (part->pane &&
                part->pane->window &&
                managed_wnd &&
                dynamic_cast<wxAuiFloatingFrame*>(managed_wnd))
            {
...
bool wxAuiManager::DoEndResizeAction(wxMouseEvent& event)
{
        ...
        m_actionPart->type == wxAuiDockUIPart::typePaneSizer)
    {
        ...
        int borrow_pane = -1;
        for (size_t i = pane_position+1; i < dock.panes.size(); ++i)
        {
        ...
        int prop_borrow = dock.panes[borrow_pane]->dock_proportion;
        ...
        dock.panes[borrow_pane]->dock_proportion = prop_borrow;
...
void wxAuiManager::OnLeftUp(wxMouseEvent& event)
{
    if (m_action == actionResize)
    {
        ...
        if (m_currentDragItem != -1 && HasLiveResize())
            m_actionPart = &m_uiParts[m_currentDragItem];
...
void wxAuiManager::OnMotion(wxMouseEvent& event)
{
    ...
    if (m_action == actionResize)
    {
        ...
        if (m_currentDragItem != -1)
            ...
            m_currentDragItem = std::distance(m_uiParts.begin(), std::ranges::find(m_uiParts, *m_actionPart));
    ...
    else if (m_action == actionDragFloatingPane)
    {
        if (m_actionWindow)
        {
            ...
            if (!dynamic_cast<wxAuiFloatingFrame*>(m_actionWindow))
            {
...
{
    wxASSERT_MSG(evt.pane, "Pane Info passed to wxAuiManager::OnPaneButton must be non-null");

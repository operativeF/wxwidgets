...
{
#if wxUSE_MDI
    if (dynamic_cast<wxAuiMDIChildFrame*>(wnd))
    {
...
void wxAuiTabCtrl::OnLeftDown(wxMouseEvent& evt)
{
    ...
    if (TabHitTest(evt.m_x, evt.m_y, &wnd))
    {
        ...
        if ((new_selection != GetActivePage() ||
            dynamic_cast<wxAuiNotebook*>(GetParent())) && !m_hoverButton)
        {
...
void wxAuiTabCtrl::OnChar(wxKeyEvent& event)
{
    ...
    if (key == WXK_TAB || key == WXK_PAGEUP || key == WXK_PAGEDOWN)
    {
        ...
        wxAuiNotebook* nb = dynamic_cast<wxAuiNotebook*>(GetParent());
        if (!nb)
        {
            event.Skip();
            return;
...
void wxAuiNotebook::OnTabDragMotion(wxAuiNotebookEvent& evt)
{
    ...
    if (m_flags & wxAUI_NB_TAB_EXTERNAL_MOVE)
    {
        ...
        if (!dynamic_cast<wxFrame*>(tab_ctrl))
        {
            while (tab_ctrl)
            {
                if (dynamic_cast<wxAuiTabCtrl*>(tab_ctrl))
                    break;
...
#if wxUSE_MDI
            if (dynamic_cast<wxAuiMDIChildFrame*>(close_wnd))
            {
                close_wnd->Close();
            }
            else

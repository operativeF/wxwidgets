...
void wxWindowDCImpl::DoDrawArc( wxCoord x1, wxCoord y1, wxCoord x2, wxCoord y2, wxCoord xc, wxCoord yc )
{
    ...
    if (m_pen.IsOk() && m_pen.GetStyle () != wxPenStyle::Transparent)
    {
        if (m_autoSetting)
            SetPen (m_pen);
...
void wxWindowDCImpl::SetPen( const wxPen &pen )
{
    ...
    if (m_currentStyle == wxPenStyle::Stipple)
    ...
    else if (m_currentStyle == wxPenStyle::Stipple && m_currentStipple.IsOk()

...
void wxGenericCalendarCtrl::SetWindowStyleFlag(unsigned int style)
{
    ...
    wxASSERT_MSG( (style & wxCAL_SEQUENTIAL_MONTH_SELECTION) ==
                  ...
                  "wxCAL_SEQUENTIAL_MONTH_SELECTION can't be changed after creation" );
...
constexpr int VERT_MARGIN =    5;           // distance between combo and calendar
constexpr int HORZ_MARGIN =    5;           //                            spin
...
void wxGenericCalendarCtrl::OnPaint([[maybe_unused]] wxPaintEvent& event)
{
    ...
    wxCoord y = 0;
    const wxCoord x0 = m_calendarWeekWidth;
    ...
    if ( HasFlag(wxCAL_SEQUENTIAL_MONTH_SELECTION) )
    {
        ...
        std::string headertext = m_date.Format("%B %Y");
        ...
        const int arrowheight = monthh / 2;
        ...
        const wxCoord arrowy = (m_heightRow - arrowheight) / 2;
        const wxCoord larrowx = (m_widthCol - (arrowheight / 2)) / 2 + x0;
        const wxCoord rarrowx = ((m_widthCol - (arrowheight / 2)) / 2) + m_widthCol*6 + x0;
        m_leftArrowRect = m_rightArrowRect = wxRect(0,0,0,0);

        if ( AllowMonthChange() )
        {
            const wxDateTime ldpm = wxDateTime(1,m_date.GetMonth(), m_date.GetYear()) - wxDateSpan::Day(); // last day prev month
            ...
            const wxDateTime fdnm = wxDateTime(1,m_date.GetMonth(), m_date.GetYear()) + wxDateSpan::Month(); // first day next month
    ...
    if ( IsExposed(x0, y, x0 + 7*m_widthCol, m_heightRow) )
    {
        ...
        const bool startOnMonday = WeekStartsOnMonday();
            ...
            if ( IsDateShown(date) )
            {
                ...
                if ( !isSel && attr && attr->HasBorder() )
                {
                    ...
                    switch ( attr->GetBorder() )
                    {
                        ...
                        default:
                            wxFAIL_MSG("unknown border type");
                    }
                }
    ...
    if ( !IsDateInRange(date) )
    {
        const wxDateTime firstOOR = GetLowerDateLimit() - wxDateSpan::Day(); // first out-of-range

        wxBrush oorbrush = *wxLIGHT_GREY_BRUSH;
        oorbrush.SetStyle(wxBrushStyle::FDiagonalHatch);
    ...
    if ( !IsDateInRange(date) )
    {
        const wxDateTime firstOOR = GetUpperDateLimit() + wxDateSpan::Day(); // first out-of-range
    ...
    if ( todate >= fromdate )
    {
        ...
        if ( GetDateCoord(fromdate, &fd, &fw) && GetDateCoord(todate, &td, &tw) )
        {
            ...
            if ( ( (tw - fw) == 1 ) && ( td < fd ) )
            {
                ...
                const wxDateTime tfd = fromdate + wxDateSpan::Days(7-fd);
                const wxDateTime ftd = tfd + wxDateSpan::Day();
#if DEBUG_PAINT
...
bool wxGenericCalendarCtrl::GetDateCoord(const wxDateTime& date, int *day, int *week) const
{
    ...
    if ( IsDateShown(date) )
    {
        ...
        *day = date.GetWeekDay();
        ...
        const int targetmonth = date.GetMonth() + (12 * date.GetYear());
        const int thismonth = m_date.GetMonth() + (12 * m_date.GetYear());
        ...
        else
        {
            ...
            else // targetmonth > thismonth
            {
                ...
                const wxTimeSpan span = date - ldcm;

                const int daysfromlast = span.GetDays();
#if DEBUG_PAINT
...
void wxGenericCalendarCtrl::OnClick(wxMouseEvent& event)
{
    ...
    switch ( HitTest(event.GetPosition(), &date, &wday) )
    {
        ...
        default:
            wxFAIL_MSG("unknown hittest code");
            [[fallthrough]];
...
void wxGenericCalendarCtrl::OnWheel(wxMouseEvent& event)
{
    ...
    {
        case wxMouseWheelAxis::Vertical:
        ...
        case wxMouseWheelAxis::Horizontal:
...
void wxGenericCalendarCtrl::OnMonthChange(wxCommandEvent& event)
{
    ...
    const wxDateTime::Month mon = (wxDateTime::Month)event.GetInt();
...
{
    wxCHECK_RET( day > 0 && day < 32, "invalid day in SetHoliday" );
...
{
    wxCHECK_RET( day > 0 && day < 32, "invalid day in Mark" );

...
wxGridCoordsToAttrMap::key_type CoordsToKey(int row, int col)
{
    ...
    // negative coords.
    return (static_cast<wxULongLong_t>(row) << 32) + wx::narrow_cast<std::uint32_t>(col);
}
...
{
    UpdateCellAttrRowsOrCols(m_attrs, wx::narrow_cast<int>(pos), numRows, 0);
}

void wxGridCellAttrData::UpdateAttrCols( size_t pos, int numCols )
{
    UpdateCellAttrRowsOrCols(m_attrs, wx::narrow_cast<int>(pos), 0, numCols);
}
...
void wxGrid::CalcWindowSizes()
{
    ...
    int gw = client_size.x - m_rowLabelWidth - fgw;
    int gh = client_size.y - m_colLabelHeight - fgh;
    if (gw < 0)
        gw = 0;
    if (gh < 0)
        gh = 0;
...
void wxGrid::ProcessRowLabelMouseEvent( wxMouseEvent& event, wxGridRowLabelWindow* rowLabelWin )
{
    ...
    {
        ChangeCursorMode(CursorMode::SelectCell, rowLabelWin);
    }
    ...
    else if ( event.LeftDown() )
    {
        ...
        if ( row != wxNOT_FOUND && CanDragRowSize(row) )
        {
            ...
            ChangeCursorMode(CursorMode::ResizeRow, rowLabelWin);
        }
        else // not a request to start resizing
        {
                 ...
                 SendEvent( wxEVT_GRID_LABEL_LEFT_CLICK, row, -1, event ) == Event_Unhandled )
            {
                        ...
                        m_selection->GetSelectionMode() != wxGridSelectColumns )
                {
                    ...
                    ChangeCursorMode(CursorMode::SelectRow, rowLabelWin);
                }
            }
        }
    }
    ...
    else if (event.LeftDClick() )
    {
        ...
        if ( row != wxNOT_FOUND && CanDragRowSize(row) )
        {
            ...
            ChangeCursorMode(CursorMode::SelectCell, GetColLabelWindow());
            m_dragLastPos = -1;
        }
    ...
    {
        if ( m_cursorMode == CursorMode::ResizeRow )
    ...
    else if ( event.Moving() )
    {
        ...
        {
            if ( m_cursorMode == CursorMode::SelectCell )
            {
                if ( CanDragRowSize(dragRowOrCol) )
                {
                    ChangeCursorMode(CursorMode::ResizeRow, rowLabelWin, false);
                }
            }
        }
        else if ( m_cursorMode != CursorMode::SelectCell )
        {
            ChangeCursorMode(CursorMode::SelectCell, rowLabelWin, false);
        }
    }
...
void wxGrid::ProcessColLabelMouseEvent( wxMouseEvent& event, wxGridColLabelWindow* colLabelWin )
{
    ...
    CalcGridWindowUnscrolledPosition(event.GetPosition().x, 0, &x, nullptr, gridWindow);
    ...
    if ( event.Dragging() )
    {
        if (!m_isDragging)
        {
            ...
            if ( m_cursorMode == CursorMode::MoveCol && col != -1 )
                DoStartMoveCol(col);
        }
            ...
            switch ( m_cursorMode )
            {
                ...
                case CursorMode::MoveCol:
                {
                    ...
                    if ( markerX != m_dragLastPos )
                    {
                        ...
                        int cli_h = GetColLabelWindow()->GetClientSize().y;
                        ...
                        if ( m_dragLastPos >= 0 )
                        {
                            ...
                            dc.SetPen(pen);
                            dc.DrawLine( m_dragLastPos + 1, 0, m_dragLastPos + 1, cli_h );
                            dc.SetPen(wxNullPen);
                        ...
                        dc.DrawLine( markerX, 0, markerX, cli_h );
    ...
    if ( event.Entering() || event.Leaving() )
    {
        ChangeCursorMode(CursorMode::SelectCell, colLabelWin);
    }
    ...
    else if ( event.LeftDown() )
    {
        ...
        if ( colEdge != wxNOT_FOUND && CanDragColSize(colEdge) )
        {
            ...
            ChangeCursorMode(CursorMode::ResizeCol, colLabelWin);
        }
        else // not a request to start resizing
        {
            if ( col >= 0 &&
                 SendEvent( wxEVT_GRID_LABEL_LEFT_CLICK, -1, col, event ) == Event_Unhandled )
            {
                if ( m_canDragColMove )
                {
                    ...
                    ChangeCursorMode(CursorMode::MoveCol, GetColLabelWindow());
                }
                else
                {
                    if ( m_selection && m_numRows > 0 && m_numCols > 0 &&
                            m_selection->GetSelectionMode() != wxGridSelectRows )
                    {
                        ...
                        ChangeCursorMode(CursorMode::SelectCol, colLabelWin);
                    }
                }
            }
        }
    ...
    else if ( event.Moving() )
    {
        ...
        if ( dragRowOrCol >= 0 )
        {
            if ( m_cursorMode == CursorMode::SelectCell )
            {
                ...
                {
                    ChangeCursorMode(CursorMode::ResizeCol, colLabelWin, false);
                }
            }
        }
        else if ( m_cursorMode != CursorMode::SelectCell )
        {
            ChangeCursorMode(CursorMode::SelectCell, colLabelWin, false);
        }
    }
...
{
    if ( m_isDragging &&
         (m_cursorMode == CursorMode::SelectCell ||
          m_cursorMode == CursorMode::SelectRow ||
          m_cursorMode == CursorMode::SelectCol) )
    {
        ...
        case CursorMode::ResizeCol:
            {
                ...
                if ( m_cursorMode == CursorMode::ResizeCol )
                         ...
                         wxGridWindow* gridWindow)
{
    ...
    else if ( m_cursorMode == CursorMode::ResizeRow )
    {
        ChangeCursorMode(CursorMode::SelectCell);
        DoEndDragResizeRow(event, gridWindow);
    }
    else if ( m_cursorMode == CursorMode::ResizeCol )
    {
        ChangeCursorMode(CursorMode::SelectCell);
        DoEndDragResizeCol(event, gridWindow);
    }
                             ...
                             wxGridWindow* gridWindow)
{
    ...
    if ( dragCol >= 0 && CanDragGridColEdges() && CanDragColSize(dragCol) )
    {
        if ( m_cursorMode != CursorMode::ResizeCol )
        {
            ChangeCursorMode(CursorMode::ResizeCol, gridWindow, false);
        }
    }
    else if ( dragRow >= 0 && CanDragGridRowEdges() && CanDragRowSize(dragRow) )
    {
        if ( m_cursorMode != CursorMode::ResizeRow)
        {
            ChangeCursorMode(CursorMode::ResizeRow, gridWindow, false);
        }
    }
    else // Neither on a row or col edge
    {
        if ( m_cursorMode != CursorMode::SelectCell )
        {
            ChangeCursorMode(CursorMode::SelectCell, gridWindow, false);
        }
    }
}
...
bool wxGrid::FreezeTo(int row, int col)
{
    ...
    if ( row > m_numFrozenRows || col > m_numFrozenCols )
    {
        ...
        client_size.x -= m_rowLabelWidth;
        client_size.y -= m_colLabelHeight;

        if ((row > 0 && GetRowBottom(row - 1) >= client_size.y) ||
            (col > 0 && GetColRight(col - 1) >= client_size.x))
            return false;
...
void wxGrid::DrawGridSpace( wxDC& dc, wxGridWindow *gridWindow )
{
  ...
  CalcGridWindowUnscrolledPosition(client_size.x + offset.x, client_size.y + offset.y, &right, &bottom, gridWindow);
  ...
  if ( right > rightCol || bottom > bottomRow )
  {
      ...
      if ( right > rightCol )
      {
          dc.DrawRectangle( rightCol, top, right - rightCol, client_size.y );
      }

      if ( bottom > bottomRow )
      {
          dc.DrawRectangle( left, bottomRow, client_size.x, bottom - bottomRow );
      }
  }
}
...
void wxGrid::DrawLabelFrozenBorder(wxDC& dc, wxWindow *window, bool isRow)
{
    if ( window )
    {
        wxSize win_size = window->GetClientSize();

        dc.SetPen(wxPen(m_gridFrozenBorderColour,
                        m_gridFrozenBorderPenWidth));

        if ( isRow )
            dc.DrawLine(0, win_size.y, win_size.x, win_size.y);
        else
            dc.DrawLine(win_size.x, 0, win_size.x, win_size.y);
    }
}
...
void wxGrid::DrawAllGridWindowLines(wxDC& dc, const [[maybe_unused]] wxRegion & reg, wxGridWindow *gridWindow)
{
    ...
    {
        wxRegion clippedcells(0, 0, client_size.x, client_size.y);
...
bool wxGrid::IsVisible( int row, int col, bool wholeCellVisible ) const
{
    ...
    if ( wholeCellVisible )
    {
        ...
        return ( left >= 0 && right <= client_size.x &&
                 top >= 0 && bottom <= client_size.y );
    }
    else
    {
        // is the cell partly visible ?
        //
        return ( ((left >= 0 && left < client_size.x) || (right > 0 && right <= client_size.x)) &&
                 ((top >= 0 && top < client_size.y) || (bottom > 0 && bottom <= client_size.y)) );
    }
}
...
void wxGrid::MakeCellVisible( int row, int col )
{
    ...
    if ( processRow )
    {
        ...
        int ch = gridWindow->GetClientSize().y;
    ...
    if ( processCol )
    {
        ...
        int cw = gridWindow->GetClientSize().x;
...
void wxGrid::SetRowLabelSize( int width )
{
    ...
    {
        width = CalcColOrRowLabelAreaMinSize(wxGridDirection::Row);
    }
...
void wxGrid::SetColLabelSize( int height )
{
    ...
    {
        height = CalcColOrRowLabelAreaMinSize(wxGridDirection::Column);
    }
...
{
    const bool column = direction == wxGridDirection::Column;
    ...
    if ( column )
    {
        ...
        if ( ShouldRefresh() )
        {
            ...
            {
                int dummy;
                int cw = m_gridWin->GetClientSize().x;
    ...
    else
    {
        ...
        {
            int dummy;
            int ch = m_gridWin->GetClientSize().y;
...
wxCoord wxGrid::CalcColOrRowLabelAreaMinSize(wxGridDirection direction)
{
    ...
    const bool calcRows = direction == wxGridDirection::Row;

                      ...
                      )
{
    ...
    icon_text->Add( text, 1, wxCENTER | wxDirection::wxLEFT, 20 );
    topsizer->Add( icon_text, 0, wxEXPAND | wxALL, 10 );

    topsizer->Add( m_text, 1, wxEXPAND | wxDirection::wxLEFT | wxDirection::wxRIGHT, 10 );

    wxBoxSizer *bottom = new wxBoxSizer( wxHORIZONTAL );
    if (isPda)
        topsizer->Add( m_checkbox, 0, wxCENTER | wxDirection::wxTOP );
    else
        bottom->Add( m_checkbox, 0, wxCENTER );

    if (!isPda)
        bottom->Add( 10,10,1 );
    bottom->Add( btnNext, 0, wxCENTER | wxDirection::wxLEFT, 10 );
    bottom->Add( btnClose, 0, wxCENTER | wxDirection::wxLEFT, 10 );

    if (isPda)
        topsizer->Add( bottom, 0, wxCENTER | wxDirection::wxALL, 5 );
    else
        topsizer->Add( bottom, 0, wxEXPAND | wxDirection::wxALL, 10 );

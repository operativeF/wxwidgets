...
wxSpinCtrlGenericBase::~wxSpinCtrlGenericBase()
{
    ...
    if (m_textCtrl)
    {
        ...
        dynamic_cast<wxSpinCtrlTextGeneric*>(m_textCtrl)->m_spin = nullptr;
...
void wxSpinCtrlGenericBase::OnSpinButton(wxSpinEvent& event)
{
    ...
    if ( DoSetValue(value, SendEvent::Text) )
        DoSendEvent();
}

void wxSpinCtrlGenericBase::OnTextLostFocus(wxFocusEvent& event)
{
    if ( SyncSpinToText(SendEvent::Text) )
        DoSendEvent();
...
void wxSpinCtrlGenericBase::OnTextChar(wxKeyEvent& event)
{
    ...
    if ( DoSetValue(value, SendEvent::None) )
        DoSendEvent();
}
...
void wxSpinCtrlGenericBase::DoSetRange(double min, double max)
{
    ...
    m_min = min;
    if ( m_value < m_min )
        DoSetValue(m_min, SendEvent::None);
    m_max = max;
    if ( m_value > m_max )
        DoSetValue(m_max, SendEvent::None);
...
void wxSpinCtrlGenericBase::SetSnapToTicks(bool snap_to_ticks)
{
    ...
    DoSetValue(m_value, SendEvent::None);
}

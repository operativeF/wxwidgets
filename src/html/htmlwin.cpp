...
bool wxHtmlWindow::LoadPage(const std::string& location)
{
    ...
    else // moving to another page
    {
        ...
        wxFSFile *f = m_Parser->OpenURL(wxHtmlURLType::Page, location);
        ...
        if (f == nullptr)
        {
            ...
            f = m_Parser->OpenURL(wxHtmlURLType::Page, location2);
        }
...
void wxHtmlWindow::CreateLayout()
{
    ...
    else // Do show scrollbars if necessary.
    {
        ...
        ShowScrollbars(wxScrollbarVisibility::Default, wxScrollbarVisibility::Always);
...
bool wxHtmlWindow::CopySelection(ClipboardType t)
{
    ...
    if ( m_selection )
    {
        ...
        wxTheClipboard->UsePrimarySelection(t == ClipboardType::Primary);
#else // !__UNIX__
...
void wxHtmlWindow::OnPaint(wxPaintEvent& WXUNUSED(event))
{
    ...
    if ( dc != &dcPaint )
    {
        ...
        dcPaint.Blit(wxPoint{0, rect.GetTop()},
                     wxSize{sz.x, rect.GetBottom() - rect.GetTop() + 1},
                     dc,
                     wxPoint{0, rect.GetTop()});
    }
}
...
void wxHtmlWindow::OnMouseUp(wxMouseEvent& event)
{
    ...
    if ( m_makingSelection )
    {
        ...
        if ( m_selection )
        {
            CopySelection(ClipboardType::Primary);
...
void wxHtmlWindow::OnDoubleClick(wxMouseEvent& event)
{
    ...
    if ( IsSelectionEnabled() )
    {
        ...
        std::ignore = CopySelection(ClipboardType::Primary);

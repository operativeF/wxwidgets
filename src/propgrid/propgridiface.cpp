...
WX_PG_IMPLEMENT_VARIANT_DATA(wxPoint)
WX_PG_IMPLEMENT_VARIANT_DATA(wxSize)
WX_PG_IMPLEMENT_VARIANT_DATA_DUMMY_EQ(wxArrayInt)
IMPLEMENT_VARIANT_OBJECT(wxFont)
...
wxString wxPropertyGridInterface::SaveEditableState( int includedStates ) const
{
    ...
    for (std::vector<wxPropertyGridPageState*>::const_iterator it_ps = pageStates.begin();
...
bool wxPropertyGridInterface::RestoreEditableState( const wxString& src, int restoreStates )
{
    ...
    pg->Freeze();
    std::vector<wxString> pageStrings = wx::utils::StrSplitEscape(src, '|', '\\');
    ...
    for ( pageIndex=0; pageIndex<pageStrings.size(); pageIndex++ )
    {
        ...
        std::vector<wxString> kvpairStrings = wx::utils::StrSplitEscape(pageStrings[pageIndex], ';', '\\');
        ...
        for ( size_t i=0; i<kvpairStrings.size(); i++ )
        {
            ...
            if ( eq_pos != wxNOT_FOUND )
            {
                ...
                std::vector<wxString> values = wx::utils::StrSplitEscape(value, ',', '\\');

...
constexpr int PWC_CHILD_SUMMARY_LIMIT =         16; // Maximum number of children summarized in a parent property's
                                           // value field.

constexpr int PWC_CHILD_SUMMARY_CHAR_LIMIT =    64; // Character limit of summary field when not editing
...
#if wxPG_USE_RENDERER_NATIVE
    wxASSERT_MSG( win, "Invalid window to be drawn" );
    ...
    if ( column == 1 )
    {
        ...
        if ( !isUnspecified )
        {
            ...
            if ( imageSize.x > 0 )
            {
                ...
                dc.SetPen( wxPen(propertyGrid->GetCellTextColour(), 1, wxPenStyle::Solid) );
            ...
            if ( propertyGrid->GetColumnCount() <= 2 )
            {
                ...
                if ( !unitsString.empty() )
                    text = wxString::Format("%s %s", text, unitsString );
            }
        }
                                   ...
                                   wxPropertyGrid* propgrid )
{
    ...
    bool parentIsRoot = dynamic_cast<wxPGRootProperty*>(parent) != nullptr;
...
{
    wxCHECK_RET( GetGrid(),
                 "Cannot obtain display info for detached property" );
...
wxString wxPGProperty::GetValueAsString( int argFlags ) const
{
    wxPropertyGrid* pg = GetGrid();
    wxCHECK_MSG( pg, {},
                 "Cannot get valid value for detached property" );
...
bool wxPGProperty::StringToValue( wxVariant& v, const wxString& text, int argFlags ) const
{
    ...
    wxLogTrace("propgrid",
               ">> %s.StringToValue('%s')", GetLabel(), text);
...
void wxPGProperty::OnCustomPaint( wxDC& dc,
                                  ...
                                  wxPGPaintData& )
{
    wxCHECK_RET( m_valueBitmap && m_valueBitmap->IsOk(), "invalid bitmap" );
...
void wxPGProperty::SetValue( wxVariant value, wxVariant* pList, int flags )
{
    ...
    if ( !value.IsNull() )
    {
        ...
        if ( value.IsType(wxPG_VARIANT_TYPE_LIST) )
        {
            ...
            value = newValue;
            //wxLogDebug(">> %s.SetValue() adapted list value to type '%s'",GetName(),value.GetType());
        }
        ...
        if ( pList && !pList->IsNull() )
        {
            ...
            //wxLogDebug(">> %s.SetValue() pList parsing",GetName());
            ...
            for ( node = list.begin(); node != list.end(); ++node )
            {
                ...
                if ( child )
                {
                    //wxLogDebug("%i: child = %s, childValue.GetType()=%s",i,child->GetBaseName(),childValue.GetType());
...
{
    wxCHECK_RET( GetGrid(),
                 "Cannot store pending value for detached property");
...
const wxPGCell& wxPGProperty::GetCell( unsigned int column ) const
{
    ...
    wxPropertyGrid* pg = GetGrid();
    wxASSERT_MSG( pg,
                  "Cannot get cell for detached property" );
    if ( !pg )
    {
...
wxVariant wxPGProperty::GetAttributesAsList() const
{
    ...
    wxVariant v( tempList, wxString::Format("@%s@attr",m_name) );
...
static const struct // FIXME: Use narrow strings.
{
    wxPGProperty::FlagType  m_flag;
    const wchar_t* m_name;
} gs_propFlagToString[4] =
{ { wxPG_PROP_DISABLED,  L"DISABLED"  },
  { wxPG_PROP_HIDDEN,    L"HIDDEN"    },
  { wxPG_PROP_NOEDITOR,  L"NOEDITOR"  },
  { wxPG_PROP_COLLAPSED, L"COLLAPSED" } };
...
{
    wxCHECK_RET( m_choices.IsOk(), "invalid choiceinfo" );
...
const wxPGEditor* wxPGProperty::GetEditorClass() const
{
    ...
    if ( GetDisplayedCommonValueCount() )
    {
        ...
        if ( dynamic_cast<wxPGTextCtrlAndButtonEditor*>(editor) )
            editor = wxPGEditor_ChoiceAndButton;

        // TextCtrl -> ComboBox
        else if ( dynamic_cast<wxPGTextCtrlEditor*>(editor) )
...
{
    wxCHECK_RET( GetGrid(),
                 "Cannot set image for detached property" );
...
bool wxPGProperty::IsVisible() const
{
    ...
    for ( parent = GetParent(); parent != nullptr; parent = parent->GetParent() )
    {
...
int wxPGProperty::GetY() const
{
    wxPropertyGrid *pg = GetGrid();
    wxCHECK_MSG( pg, 0,
                 "Cannot obtain coordinates of detached property" );
...
void wxPGProperty::AdaptListToValue( wxVariant& list, wxVariant* value ) const
{
    ...
    *value = GetValue();
    ...
    //wxLogDebug(">> %s.AdaptListToValue()",GetBaseName());
    ...
    for ( unsigned int i = 0; i < GetChildCount(); i++ )
    {
        ...
        {
            //wxLogDebug("  %s(n=%i), %s",childValue.GetName(),n,childValue.GetType());
                                        ...
                                        unsigned int* nextItemY ) const
{
    ...
    *nextItemY = iy;
    ...
    if ( current )
    {
        wxLogDebug("%s::GetItemAtY(%i) -> %s",this->GetLabel(),y,current->GetLabel());
    }
    else
    {
        wxLogDebug("%s::GetItemAtY(%i) -> NULL",this->GetLabel(),y);
    }
    */
...
wxPGProperty* wxPGProperty::GetItemAtY( unsigned int y ) const
{
    ...
    wxCHECK_MSG( pg, nullptr,
                 "Cannot obtain property item for detached property" );
...
bool wxPGProperty::IsTextEditable() const
{
    ...
    if ( HasFlag(wxPG_PROP_NOEDITOR) &&
         (GetChildCount() ||
          wxString(GetEditorClass()->wxGetClassInfo()->wxGetClassName()).EndsWith("Button"))
       )
        return false;
...
void wxPGChoices::Add( const std::vector<std::string>& arr, const std::vector<int>& arrint )
{
    ...
    wxASSERT_MSG( valcount >= itemcount || valcount == 0,
                  "Insufficient number of values in the array" );

...
constexpr int wxPG_YSPACING_MIN =               1;
constexpr int wxPG_DEFAULT_VSPACING =           2; // This matches .NET propertygrid's value,
...
// adjusted.
constexpr int IN_CELL_EXPANDER_BUTTON_X_ADJUST =    2;
...
wxPropertyGrid::~wxPropertyGrid()
{
    ...
    if ( HasExtraStyle(wxPG_EX_ENABLE_TLP_TRACKING) )
    {
        ...
        wxASSERT_MSG( !IsEditorsValueModified(),
                      "Most recent change in property editor was "
                      "lost!!! (if you don't want this to happen, "
                      "close your frames and dialogs using "
                      "Close(false).)" );
    }
                                         ...
                                         bool editable )
{
    ...
    (
         column != 1,
         "Set wxPG_PROP_READONLY property flag instead"
    );
                                       ...
                                       int selFlags )
{
    ...
    wxCHECK_RET(selected, "No property selected");
    wxCHECK_RET(colIndex != 1, "Do not use this for column 1");
                                 ...
                                 bool forceDifferent = false)
{
    ...
    wxCHECK_MSG( isinside < 3,
                 *wxBLACK,
                 "wxPGAdjustColour should not be recursively called more than once" );
                                                     ...
                                                     const wxSize& sz )
{
    ...
    wxCHECK_MSG( y >= 0, wxDefaultPosition, "invalid y?" );
                                        ...
                                        int flags )
{
    ...
    if ( flags & SendEvtChanging )
    {
        ...
        if ( evtChangingProperty->HasFlag(wxPG_PROP_COMPOSED_VALUE) )
        {
            ...
            {
                wxLogDebug("WARNING: wxEVT_PG_CHANGING is about to happen with old value.");
            }
        }
    }
...
bool wxPropertyGrid::DoSelectProperty( wxPGProperty* p, unsigned int flags )
{
    ...
    if (p)
    {
        wxLogDebug("SelectProperty( %s (%s[%i]) )",p->GetLabel(),
            p->m_parent->GetLabel(),p->GetIndexInParent());
    }
    else
    {
        wxLogDebug("SelectProperty( NULL, -1 )");
    }
    */
                        ...
                        else if ( m_colHover == 1 && !m_propHover->IsValueUnspecified())
                        {
                            ...
                            if ( GetColumnCount() <= 2 )
                            {
                                ...
                                if ( !unitsString.empty() )
                                    tipString = wxString::Format("%s %s", tipString, unitsString );
                            }
                        }
...
void wxPropertyGrid::OnIdle( [[maybe_unused]] wxIdleEvent& event )
{
    ...
    while ( cntAfter > 0 )
    {
        ...
        wxASSERT_MSG( cntAfter <= cntBefore,
            "Increased number of pending items after deletion" );
    ...
    while ( cntAfter > 0 )
    {
        ...
        cntAfter = m_removedProperties.size();
        wxASSERT_MSG( cntAfter <= cntBefore,
            "Increased number of pending items after removal" );
                                            ...
                                            wxPGChoices* pChoices )
{
    ...
    {
        ProcessError(wxString::Format("new children cannot be added to '%s'",parent->GetName()));
        return nullptr;
    }

    if ( !classInfo || !classInfo->IsKindOf(wxCLASSINFO(wxPGProperty)) )
    {
        ProcessError(wxString::Format("'%s' is not valid property class",propClass));
        return nullptr;
    }
                                                   ...
                                                   const wxString& idString )
{
    ...
    if ( choicesString[0] == wxT('@') )
    {
        ...
        if ( it == m_dictIdChoices.end() )
            ProcessError(wxString::Format("No choices defined for id '%s'",ids));
        else
                                            ...
                                            const wxString& value )
{
    ...
    if ( type.empty() )
    {
        ...
        // Auto-detect type
        if ( valuel == "true" || valuel == "yes" || valuel == "1" )
            variant = true;
        else if ( valuel == "false" || valuel == "no" || valuel == "0" )
            variant = false;
    ...
    else
    {
        ...
        {
            ProcessError(wxString::Format("Invalid attribute type '%s'",type));
            return false;
        }
    }

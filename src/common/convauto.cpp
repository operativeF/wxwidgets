...
const char* wxConvAuto::GetBOMChars(wxBOM bom, size_t* count)
{
    ...
    switch ( bom )
    {
        case wxBOM::UTF32BE: *count = WXSIZEOF(BOM_UTF32BE); return BOM_UTF32BE;
        case wxBOM::UTF32LE: *count = WXSIZEOF(BOM_UTF32LE); return BOM_UTF32LE;
        case wxBOM::UTF16BE: *count = WXSIZEOF(BOM_UTF16BE); return BOM_UTF16BE;
        case wxBOM::UTF16LE: *count = WXSIZEOF(BOM_UTF16LE); return BOM_UTF16LE;
...
wxBOM wxConvAuto::DetectBOM(const char *src, size_t srcLen)
{
    ...
    switch ( srcLen )
    {
        ...
        case 3:
            ...
            {
                if ( srcLen == 3 )
                    return src[2] == '\xBF' ? wxBOM::UTF8 : wxBOM::None;

                return wxBOM::Unknown;
            }

            if ( src[0] == '\xFE' && src[1] == '\xFF' )
                return wxBOM::UTF16BE;
        ...
        default:
            ...
            if ( src[0] == '\x00' && src[1] == '\x00' &&
                 src[2] == '\xFE' && src[3] == '\xFF' )
                return wxBOM::UTF32BE;

            if ( src[0] == '\xFF' && src[1] == '\xFE' &&
                 src[2] == '\x00' && src[3] == '\x00' )
                return wxBOM::UTF32LE;
                    ...
                    const char *src, size_t srcLen) const
{
    ...
    if ( rc == wxCONV_FAILED && m_bomType == wxBOM::None && !m_ownsConv )
    {

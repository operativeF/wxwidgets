...
constexpr wxChar wxSelectDispatcher_Trace[] = "selectdispatcher";
...
{
    wxCHECK_MSG( fd >= 0, false, "invalid descriptor" );
    ...
    for ( int n = 0; n < Max; n++ )
    {
        if ( wxFD_ISSET(fd, const_cast<fd_set*>(&m_fds[n])) )
        {
            wxLogTrace(wxSelectDispatcher_Trace,
                       "Got %s event on fd %d", ms_names[n], fd);
...
bool wxSelectDispatcher::RegisterFD(int fd, wxFDIOHandler *handler, int flags)
{
    ...
    wxLogTrace(wxSelectDispatcher_Trace,
                "Registered fd %d: input:%d, output:%d, exceptional:%d", fd, (flags & wxFDIO_INPUT) == wxFDIO_INPUT, (flags & wxFDIO_OUTPUT), (flags & wxFDIO_EXCEPTION) == wxFDIO_EXCEPTION);
    return true;
}
...
bool wxSelectDispatcher::ModifyFD(int fd, wxFDIOHandler *handler, int flags)
{
    ...
    wxASSERT_MSG( fd <= m_maxFD, "logic error: registered fd > m_maxFD?" );

    wxLogTrace(wxSelectDispatcher_Trace,
                "Modified fd %d: input:%d, output:%d, exceptional:%d", fd, (flags & wxFDIO_INPUT) == wxFDIO_INPUT, (flags & wxFDIO_OUTPUT) == wxFDIO_OUTPUT, (flags & wxFDIO_EXCEPTION) == wxFDIO_EXCEPTION);
...
bool wxSelectDispatcher::UnregisterFD(int fd)
{
    ...
    wxLogTrace(wxSelectDispatcher_Trace,
                "Removed fd %d, current max: %d", fd, m_maxFD);
    return true;
}
...
int wxSelectDispatcher::ProcessSets(const wxSelectSets& sets)
{
    ...
    for ( int fd = 0; fd <= m_maxFD; fd++ )
    {
        ...
        if ( !handler )
        {
            wxFAIL_MSG( "NULL handler in wxSelectDispatcher?" );
            continue;
        }

...
bool wxGBSizerItem::SetPos( const wxGBPosition& pos )
{
    if (m_gbsizer)
    {
        wxCHECK_MSG( !m_gbsizer->CheckForIntersection(pos, m_span, this), false,
                 "An item is already at that position" );
    }
    m_pos = pos;
    return true;
}
...
wxGBPosition wxGridBagSizer::GetItemPosition(wxWindow *window)
{
    ...
    wxCHECK_MSG(item, badpos, "Failed to find item.");
    return item->GetPos();
}
...
wxGBPosition wxGridBagSizer::GetItemPosition(size_t index)
{
    ...
    wxCHECK_MSG( node, badpos, "Failed to find item." );
...
{
    wxGBSizerItem* item = FindItem(window);
    wxCHECK_MSG(item, false, "Failed to find item.");
...
{
    wxSizerItemList::compatibility_iterator node = m_children.Item( index );
    wxCHECK_MSG( node, false, "Failed to find item." );
    wxGBSizerItem* item = (wxGBSizerItem*)node->GetData();
    return item->SetPos(pos);
}
...
{
    wxGBSizerItem* item = FindItem(window);
    wxCHECK_MSG( item, wxGBSpan::Invalid(), "Failed to find item." );
    return item->GetSpan();
}
...
{
    wxSizerItemList::compatibility_iterator node = m_children.Item( index );
    wxCHECK_MSG( node, wxGBSpan::Invalid(), "Failed to find item." );
    wxGBSizerItem* item = (wxGBSizerItem*)node->GetData();
    return item->GetSpan();
}
...
{
    wxFAIL_MSG("Invalid Add form called.");
    return nullptr;
}

wxSizerItem* wxGridBagSizer::Prepend( wxWindow *, int, int, int, wxObject*  )
{
    wxFAIL_MSG("Prepend should not be used with wxGridBagSizer.");
    return nullptr;
}
...
{
    wxFAIL_MSG("Insert should not be used with wxGridBagSizer.");
    return nullptr;
}

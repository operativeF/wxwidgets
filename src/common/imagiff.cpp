...
namespace
{
    ...
    **************************************************************************/
    ...
    void decomprle(const byte* sptr, byte* dptr, long slen, long dlen)
    {
    ...
    /******************************************/
    unsigned int iff_getword(const byte* ptr)
    {
        unsigned int v;
    ...
    /******************************************/
    unsigned long iff_getlong(const byte* ptr)
    {
        unsigned long l;
...
int wxIFFDecoder::ReadIFF()
{
    ...
    wxLogTrace("iff", "IFF ILBM file recognized");
    ...
    if (strncmp(reinterpret_cast<const char*>(dataptr), "BMHD", 4) == 0) { // BMHD chunk?
        ...
        wxLogTrace("iff", "Read %d colors from IFF file.",
            colors);
        ...
        wxLogTrace("iff",
            "LoadIFF: %s %dx%d, planes=%d (%d cols), comp=%d",
        ...
        wxLogTrace("iff",
            "Converting CMAP from normal ILBM CMAP");
        ...
        if (fmt == ILBM_EHB) {
            wxLogTrace("iff", "Doubling CMAP for EHB mode");
        ...
        wxLogTrace("iff", "Skipping unknown chunk '%c%c%c%c'",
                            ...
                            [[maybe_unused]] wxOutputStream& stream, bool verbose)
{
    if (verbose)
    {
        wxLogDebug("IFF: the handler is read-only!!");
    }

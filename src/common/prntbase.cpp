...
wxPrintDialogBase::wxPrintDialogBase(wxWindow *parent,
                                     ...
                                     const wxSize &size,
                                     unsigned int style)
    : wxDialog( parent, id, title.empty() ? _("Print") : title,
                pos, size, style )
{
}
...
wxPageSetupDialogBase::wxPageSetupDialogBase(wxWindow *parent,
                                     ...
                                     const wxSize &size,
                                     unsigned int style)
    : wxDialog( parent, id, title.empty() ? _("Page setup") : title,
                pos, size, style )
{
}
...
wxPrintAbortDialog::wxPrintAbortDialog(wxWindow *parent,
    ...
    : wxDialog(parent, wxID_ANY, _("Printing"), pos, size, style, name) // FIXME: Use std::string
{
...
bool wxPrintout::SetUp(wxDC& dc)
{
    ...
    m_pageWidthPixels = dcSize.x;
    m_pageHeightPixels = dcSize.y;
...
void wxPrintout::MapScreenSizeToDevice()
{
    ...
    wxSize po_size = m_printoutDC->GetSize();
...
void wxPrintPreviewBase::CalcRects(wxPreviewCanvas *canvas, wxRect& pageRect, wxRect& paperRect)
{
    ...
    paperRect.x = (canvas_size.x - paperRect.width) / 2;
    if (paperRect.x < m_leftMargin)
        paperRect.x = m_leftMargin;
    paperRect.y = (canvas_size.y - paperRect.height) / 2;
...
bool wxPrintPreviewBase::RenderPage(int pageNum)
{
    ...
    if (!m_previewCanvas)
    {
        wxFAIL_MSG("wxPrintPreviewBase::RenderPage: must use wxPrintPreviewBase::SetCanvas to let me know about the canvas!");
        return false;
    }

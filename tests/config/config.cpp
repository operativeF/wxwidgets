...
{
    std::string app = "wxConfigTestCase";
    ...
    config.reset(new wxConfig(app, vendor, "", "",
                              wxCONFIG_USE_LOCAL_FILE));
    std::string string1 = config->Read("string1");
    CHECK( string1 == "abc" );
    ...
    CHECK( config->ReadLong("long1", 0L) == 234 );
    ...
    CHECK( config->ReadDouble("double1", 0.0) == double1 );
...
size_t ReadValues(const wxConfig& config, bool has_values)
{
    ...
    std::string string1 = config.Read("string1", "abc");
    read++;

    std::string string2 = config.Read("string2", "def");
    read++;
    ...
    std::string string4;
    r = config.Read("string4", &string4, std::string("def"));
    CHECK( r == has_values );
    read++;
    ...
    long long1;
    r = config.Read(std::string("long1"), &long1, 234L);
    CHECK( r == has_values );
    read++;
    ...
    // FIXME: add a formatter for wxColour
#ifdef TEST_WXCOLOUR
    //wxColour color1;
...
{
    std::string app = "wxConfigTestCaseRD";
    std::string vendor = "wxWidgets";
    std::unique_ptr<wxConfig> config(new wxConfig(app, vendor, "", "",
                                              wxCONFIG_USE_LOCAL_FILE));
    config->DeleteAll();

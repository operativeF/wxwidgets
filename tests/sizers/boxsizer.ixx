...
export module WX.Test.BoxSizer;
...
ut::suite BoxSizerTests = []
{
    using namespace ut;

    auto m_win = new wxWindow(wxTheApp->GetTopWindow(), wxID_ANY);
    auto m_sizer = new wxBoxSizer(wxHORIZONTAL);

    m_win->SetClientSize(127, 35);
    m_win->SetSizer(m_sizer);

    "BoxSizer::Size1"_test = [&]
    {
        const wxSize sizeTotal = m_win->GetClientSize();
        const wxSize sizeChild = sizeTotal / 2;

        auto child = new wxWindow(m_win, wxID_ANY, wxDefaultPosition, sizeChild);

        m_sizer->Add(child);
        m_win->Layout();
        expect(child->GetSize() == sizeChild);

        m_sizer->Clear();
        m_sizer->Add(child, wxSizerFlags(1));
        m_win->Layout();

        expect( child->GetSize() == wxSize(sizeTotal.x, sizeChild.y) );

        m_sizer->Clear();
        m_sizer->Add(child, wxSizerFlags(1).Expand());
        m_win->Layout();

        expect(child->GetSize() == sizeTotal);
    ...
    "BoxSizer::Size3"_test = [&]
    {
        ...
        for ( unsigned i = 0; i < WXSIZEOF(layoutTestData); i++ )
        {
            ...
            for ( unsigned p = 0; p < 3; p++)
            {
                ...
                for ( j = 0; j < WXSIZEOF(child); j++ )
                {
                    expect(ltd.sizes[j] == child[j]->GetSize().x) << fmt::format(
    ...
    "BoxSizer::CalcMin"_test = [&]
    {
        ...
        for ( unsigned i = 0; i < WXSIZEOF(calcMinTestData); i++ )
        {
            ...
            expect(cmtd.total == m_sizer->CalcMin().x) << fmt::format("In test #{}", i);
        }
    };

    "BoxSizer::SetMinSize"_test = [&]
    {
        auto child = new wxWindow(m_win, wxID_ANY);
        ...
        expect(m_sizer->GetMinSize().x >= 100);
...
#if wxUSE_LISTBOX
    "BoxSizer::BestSizeRespectsMaxSize"_test = [&]
    {
        m_sizer->Clear();
        ...
        expect(listbox->GetSize().GetWidth() <= maxWidth);
    };

    "BoxSizer::RecalcSizesRespectsMaxSize1"_test = [&]
    {
        m_sizer->Clear();
        ...
        expect(listbox2->GetSize().GetWidth() == maxWidth);
    };
#endif

    "BoxSizer::RecalcSizesRespectsMaxSize2"_test = [&]
    {
        m_sizer->Clear();
        ...
        expect(child1->GetSize().GetHeight() == 125);
        expect(child2->GetSize().GetHeight() == 50);
        expect(child3->GetSize().GetHeight() == 125);
    };

    "BoxSizer::Replace"_test = [&]
    {
        m_sizer->AddSpacer(1);
